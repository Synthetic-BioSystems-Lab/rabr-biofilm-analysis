...
#DADA2/16SDADA2visualization/")

gather_metadata <- function(target, page) {
  read_excel("DADA2/16SDADA2visualization/16S_D_metadata.xlsx", sheet=page) %>%
    rename_all(tolower) %>%
...
high_low %>%
  select(-dli_level) %>%
  write_tsv("DADA2/16SDADA2visualization/processed_data/16S.high_low.shared")

high_low %>%
  select(Group, dli_level) %>%
  write_tsv("DADA2/16SDADA2visualization/processed_data/16S.high_low.design")

# run mothur lefse from desktop
#in terminal
# mothur\mothur
#lefse(shared=16S.high_low.shared, design=16S.high_low.design, inputdir=DADA2/16SDADA2visualization/processed_data)
     #NOT WORKING
...
# high vs low
read_tsv("DADA2/16SDADA2visualization/processed_data/16S.high_low.0.03.lefse_summary") %>%
  drop_na(LDA) %>%
  filter(LDA > 3.5) %>%
...
ggsave("DADA2/16SDADA2visualization/16S_D_plots/16SD_lefse_pilot_dli_high_low.tiff", width=6, height=5)
...
LabvPilot %>%
  select(-section) %>%
  write_tsv("DADA2/16SDADA2visualization/processed_data/16S.labvPilot.shared")

LabvPilot %>%
  select(Group, section) %>%
  write_tsv("DADA2/16SDADA2visualization/processed_data/16S.labvPilot.design")
...
# Lab v Pilot
read_tsv("DADA2/16SDADA2visualization/processed_data/16S.labvPilot.0.03.lefse_summary") %>%
  drop_na(LDA) %>%
  filter(LDA > 4) %>%
  inner_join(., taxonomy, by=c("OTU" = "asvs")) %>%
...
TFvPilot %>%
  select(-section) %>%
  write_tsv("DADA2/16SDADA2visualization/processed_data/16S.TFvPilot.shared")

TFvPilot %>%
  select(Group, section) %>%
  write_tsv("DADA2/16SDADA2visualization/processed_data/16S.TFvPilot.design")
...
ggsave("DADA2/16SDADA2visualization/16S_D_plots/16SD_lefse_TFvpilot.tiff", width=6, height=5)
...
CVWRFvPilot %>%
  select(-section) %>%
  write_tsv("DADA2/16SDADA2visualization/processed_data/16S.CVWRFvPilot.shared")

CVWRFvPilot %>%
  select(Group, section) %>%
  write_tsv("DADA2/16SDADA2visualization/processed_data/16S.CVWRFvPilot.design")

# run mothur lefse from desktop
#in terminal
# mothur\mothur
#lefse(shared=16S.CVWRFvPilot.shared, design=16S.CVWRFvPilot.design, inputdir=DADA2\16SDADA2visualization\processed_data)
#NOT WORKING
...
# CVWRF v Pilot
read_tsv("DADA2/16SDADA2visualization/processed_data/16S.CVWRFvPilot.0.03.lefse_summary") %>%
  drop_na(LDA) %>%
  filter(LDA > 4) %>%
  inner_join(., taxonomy, by=c("OTU" = "asvs")) %>%
...
ggsave("DADA2/16SDADA2visualization/16S_D_plots/16SD_lefse_CVWRFvpilot.tiff", width=6, height=5)
