...
ggsave("16S_D_plots/16SD_alpha_div0_all.tiff", width=8, height=8)
...
ggsave("16S_D_plots/16SD_alpha_div0_pilot.tiff", width=8, height=8)
...
ggsave("16S_D_plots/16SD_alpha_div0_81RABR.tiff", width=8, height=8)
...
ggsave("16S_D_plots/16SD_alpha_div0_TF.tiff", width=8, height=8)
...
ggsave("16S_D_plots/16SD_alpha_div0_control.tiff", width=8, height=8)
...
ggsave("16S_D_plots/16SD_alpha_div_pilot_dli.tiff", width=5, height=4)
...
ggsave("16S_D_plots/16SD_alpha_div_pilotv81.tiff", width=5, height=4)
...
RnS <- date_meta_alpha %>%
  ...
  mutate(sample_id = "11_2_16S", invsimpson = ((S + R)/2)) %>%
  select(sample_id, date, invsimpson)
...
S123 <- date_meta_alpha %>%
  ...
  group_by(sample_id) %>%
  pivot_wider(names_from = sample_id, values_from = invsimpson) %>%
  mutate(sample_id = "10_12_16S", invsimpson = ((S1_16S + S2_16S + S3_16S)/3)) %>%
  select(sample_id, date, invsimpson)

mod_date_meta_alpha <- date_meta_alpha %>%
  filter(sample_id %in% c("10_5_16S", "19_16S", "26_16S")) %>%
  rbind(.,RnS) %>%
  rbind(.,S123) %>%
  ungroup() %>%
  select(sample_id, date, invsimpson)
...
ggsave("16S_D_plots/16SD_alpha_div_timeline.tiff", width=3.5, height=2.3)
