...
df_23s = df_23s[
    (df_23s["section"] == "pilot")
    & (df_23s["level"] == "Phylum")
    & (~df_23s["sample_id"].isin(["P_11_02_23_S2", "P_10_12_23_S2", "P_10_12_23_S3"]))
].copy()

print(df_23s)

# Write csv
df_23s.to_csv("23s_data_pilot.csv", index=False)

# Exclude abundunce less than 1%
stacked_df = df_23s[df_23s["rel_abund"] >= 1].copy()
...
ax.set_title("Relative Abundance of 23S Phototrophic Taxa by Sample", fontsize=14)
ax.set_xlabel("")
ax.set_ylim(0, 100)
...
plt.tight_layout()
plt.savefig("23S_pilot.pdf", format="pdf", bbox_inches="tight")
plt.show()
