...
abund <- function(tax, pl, colors){
  ...
  #   summarize(.groups="drop") %>% #mean=mean(rel_abund),
  #   group_by(taxon) %>%
  #   summarize(pool = max(rel_abund) < pl, 
  #             max = max(rel_abund),
  #             .groups="drop")
  ...
  write.csv(taxon_rel_abund,paste("16Spilot/16Scsvs/16S_abund_", tax, ".csv", sep=""), row.names = FALSE)
  ...
  write.csv(prep,paste("16Spilot/16Splots/16S_abund_", tax, ".csv", sep=""), row.names = FALSE)
  ...
  prep %>%
    ...
    facet_grid(~section, scale="free_x", space="free", labeller = labeller(section=pretty)) + 
  ...
  ggsave(paste("16Spilot/16Splots/16S_stacked_bar_", tax, ".tiff", sep=""), width=13, height=4)
  ...
  ggsave(paste("16Spilot/16Splots/16S_heat_map_", tax, ".tiff", sep=""), width=13, height=4)
}  
